...
Tiêu chí chọn service của CDO08 không phải xây một production platform đầy đủ ngay từ W12. Hạ tầng này là **evidence platform** giúp team AI chứng minh AI Engine có thể nhận telemetry thật, query window 120 phút, đưa ra recommendation, ghi audit và hiển thị kết quả trên Grafana. Vì vậy nhóm ưu tiên:

- **Chi phí thấp và bounded runtime:** compute chỉ chạy khi có workload/test window nếu có thể; tránh service luôn chạy nếu không tạo thêm evidence quan trọng.
- **Dễ vận hành và dễ debug:** ưu tiên managed/serverless service để giảm việc tự patch, tự backup, tự scale.
- **Evidence rõ:** mỗi component phải tạo được artifact chấm điểm như log, queue depth, dashboard, audit item, health check, plan/apply.
- **Đủ đúng contract, không over-build prod:** không thêm multi-region, service mesh, custom UI, tenant provisioning SaaS hoặc auto-remediation khi chưa cần cho mục tiêu chứng minh AI.

| Thành phần | Lựa chọn của CDO08 | Trách nhiệm | Lý do chọn | Kiểm soát chi phí |
|---|---|---|---|---|
| Synthetic workload | ECS Fargate task chạy generator/k6 | Tạo normal baseline, gradual drift, sudden spike, slow leak, noisy baseline cho 3 service; anomaly scenarios có 120 phút warm-up baseline | Fargate đủ để chạy k6 dài giờ trên AWS, không cần quản trị EC2, có task log/ECR tag làm evidence. Đây là workload test cho AI, không phải production traffic generator. | Chỉ chạy trong test window; giới hạn task count, CPU và memory |
| Telemetry entry | API Gateway + Lambda ingest | Nhận event, kiểm tra schema/whitelist, gắn correlation ID | API Gateway + Lambda tạo HTTP boundary rẻ, dễ bật IAM auth/throttling và log reject reason. Không cần build ECS ingest service luôn chạy như môi trường prod. | Request-size limit, throttling, không giữ compute luôn chạy |
| Buffer | Amazon SQS Standard + DLQ | Tách producer khỏi writer, giữ event khi writer/storage lỗi | SQS là queue managed rẻ, đủ retry/DLQ evidence để chứng minh không mất telemetry âm thầm. Không cần Kinesis/shard tuning khi scope chỉ là demo 3 service và 60s sample. | Retention vừa đủ demo, CloudWatch alarm khi DLQ tăng |
| Telemetry writer | Lambda đọc SQS theo batch, đóng vai Prometheus remote-write adapter | Chuyển JSON telemetry thành Prometheus metric name từ `metric_type` + labels, remote-write vào AMP; xử lý retry từng record | Lambda scale theo backlog và chỉ tốn tiền khi xử lý event; đủ để bridge telemetry sang AMP mà không phải vận hành collector/service riêng. | Giới hạn concurrency/batch; POC remote-write trước khi lock (§5.4) |
| Primary telemetry store | Amazon Managed Service for Prometheus (AMP) | Lưu/query metric theo PromQL và retention ≥90 ngày | AMP managed, query PromQL tốt cho signal window của AI và tích hợp Grafana trực tiếp. Dùng AMP giúp tập trung vào chứng minh AI thay vì tự vận hành time-series database. | Retention mặc định 150 ngày; kiểm soát label cardinality, ingest/query volume và cost |
| Prediction trigger | Amazon EventBridge Scheduler | Trigger Prediction Integration Lambda theo lịch, tách prediction cadence khỏi telemetry ingest rate | Scheduler đơn giản, rẻ và đủ để tạo cadence 5 phút/service. AI không bị gọi theo từng metric nên dễ kiểm soát chi phí và tránh overload model. | 3 schedule nhỏ, chỉ chạy mỗi 5 phút; không dùng DLQ riêng, dùng CloudWatch alarm cho invoke failure |
| AI Engine Runtime | ECS Fargate FastAPI service do CDO08 host | Nhận `POST /v1/predict`, detect anomaly và trả recommendation | ECS Fargate khớp artifact container của team AI và không yêu cầu team vận hành node/cluster. Runtime đủ cho smoke/demo/health check; không over-build EKS/service mesh khi mục tiêu là chứng minh model serving hoạt động. | CDO08 chịu cost/runtime engine; min/max task, autoscaling và scale-to-zero/cost breaker cần kiểm soát |
| AI integration | Prediction integration Lambda | Query PromQL window ≥120 phút từ AMP, map metric thành `signal_window`, gọi AI API Gateway `/v1/predict` bằng SigV4, lưu outcome và tạo annotation | Lambda adapter cô lập phần AI khỏi chi tiết PromQL/Grafana/audit, dễ test success/fallback và không cần một integration service luôn chạy. | Timeout/retry/circuit breaker rõ; cap concurrency và query window |
| Fail-open | Fallback evaluator Lambda | So sánh static threshold khi AI lỗi | Lambda fallback là đường an toàn tối thiểu để chứng minh fail-open. Nó không thay AI, chỉ đảm bảo demo vẫn có alert khi AI dependency lỗi. | Chỉ chạy khi dependency AI lỗi |
| Dashboard overlay | Amazon Managed Grafana | Dashboard metric và annotation prediction/fallback | Managed Grafana đúng yêu cầu “không build UI mới”, có annotation overlay để mentor thấy AI decision trực tiếp trên metric. Tiết kiệm effort hơn tự build frontend/dashboard. | Chỉ tạo workspace/user cần cho demo |
| Email alert | SNS Topic + email subscription | Gửi prediction/fallback alert ra email PM khi anomaly vượt ngưỡng | SNS email là notification path rẻ và nhanh để chứng minh alert có thể đi ra ngoài dashboard. Không cần Slack/PagerDuty production integration trong W12. | Người nhận phải confirm subscription; dùng cooldown để tránh spam prediction |
| Audit store | DynamoDB SSE-KMS, TTL | Lưu mỗi prediction/fallback call, query theo correlation ID | DynamoDB serverless, on-demand, query nhanh theo correlation ID; phù hợp audit lookup trong demo. Không cần RDS/Aurora khi không làm reporting SQL phức tạp. | On-demand trong capstone; TTL xóa record quá hạn |
| Secrets/encryption | Secrets Manager + KMS; AMP encryption at rest mặc định | Giữ token Grafana và runtime config AI Engine; AI auth dùng IAM SigV4 nên không dùng API key | Secrets Manager/KMS tạo evidence bảo mật rõ ràng mà không phải tự quản secret store. Chỉ dùng mức cần thiết để bảo vệ token/audit, tránh complexity vượt scope. | Chỉ dùng AMP CMK nếu security requirement cần, vì tăng IAM/KMS complexity |
| Observability | CloudWatch Logs, Metrics, Alarms | Theo dõi ingest error, DLQ, writer, AI dependency, fallback và latency | CloudWatch là path debug/evidence nhanh nhất cho Lambda/ECS/SQS. Đủ cho W12 mà không cần thêm tracing stack/collector phức tạp. | Retention log rõ, structured log, tránh debug log volume cao |
...
Phần này **không bổ sung component mới**. Nó so sánh các implementation cho từng component đã có trong §2 để làm rõ vì sao CDO08 chọn service hiện tại và vì sao không chọn các option còn lại. “Không chọn” không đồng nghĩa “service đó tệ”; nó chỉ không khớp mục tiêu W12 của CDO08. Lựa chọn cuối được đánh giá theo mục tiêu: tạo evidence E2E cho AI Engine với chi phí thấp, vận hành đơn giản và đủ đúng contract; không tối ưu cho production peak, HA dài hạn hoặc platform mở rộng nhiều tenant.

### 5.1 Synthetic workload

| Option | Điểm mạnh | Trade-off với mục tiêu CDO08 | Quyết định CDO08 |
|---|---|---|---|
...
| Lambda generator | Rất nhẹ cho event burst ngắn | Bị giới hạn runtime, không phù hợp một scenario chạy ≥2 giờ | Không chọn cho generator vì không đáp ứng test window dài. |
| EC2 tự quản | Tự do tooling, dễ SSH debug | Phải patch/terminate, có nguy cơ quên tắt gây cost | Không chọn trong capstone vì tăng vận hành và rủi ro cost. |
...
| ALB + ECS/Fargate collector | Process lâu dài, library/protocol linh hoạt | Always-on cost và vận hành service | Không chọn vì ingest không cần service luôn chạy. |
| Generator ghi trực tiếp TSDB/remote-write | Ít component, latency thấp nhất | Không có central validation/auth/retry boundary; producer chịu lỗi storage | Không chọn vì yếu về audit/failure evidence. |
...
| Kinesis Data Streams | Throughput streaming, shard ordering, nhiều consumer realtime | Shard/capacity/cost phức tạp hơn; cần chứng minh volume thực tế | Không chọn vì throughput demo không cần shard/consumer model phức tạp. |
...
| ECS/Fargate consumer với ADOT Collector/Prometheus Agent | Dùng collector chuẩn cho Prometheus remote-write và có process ổn định | Always-on cost, thêm container/config/ops | Không chọn vì Lambda writer đã đủ cho volume/test window W12. |
...
| Option | Điểm mạnh | Trade-off với mục tiêu CDO08 | Quyết định CDO08 |
|---|---|---|---|
| Amazon Timestream for LiveAnalytics | Managed time-series query và lifecycle tốt | Account capstone là new customer, AWS Console chặn truy cập | Không chọn vì không triển khai được trên account hiện tại. |
...
| InfluxDB/Prometheus tự quản trên ECS/EKS | Toàn quyền plugin/query/retention | Persistent volume, backup, patching, HA là gánh nặng W11–W12 | Không chọn do operational overhead. |
| S3 + Athena | Rẻ cho archive/analytics batch | Không đáp ứng primary time-series query requirement của TF4 | Không chọn làm primary store vì không phù hợp query window realtime cho AI. |
...
| EventBridge Event Bus | Phù hợp route event theo rule/pattern từ nhiều nguồn | Không tự tạo cadence prediction; thêm event routing không cần thiết cho scope hiện tại | Không chọn W11 vì prediction cần cadence cố định theo service. |
| SQS telemetry event trigger Prediction Lambda | Gần real-time, ít thêm service | Gọi AI theo từng metric/batch, dễ quá tải AI, tốn cost và tạo prediction trùng lặp | Không chọn vì prediction cần chạy theo cadence riêng, không chạy theo từng telemetry event. |
...
| ECS/Fargate integration service | Connection pooling, process luôn sẵn, library linh hoạt | Fixed cost và vận hành service dù call không liên tục | Không chọn vì prediction call theo lịch thấp, không cần service luôn chạy. |
| Step Functions orchestration | Visual retry/stateful workflow | Thêm service/latency/cost, quá nặng cho một prediction call | Không chọn vì flow prediction chỉ cần một Lambda adapter có retry/fallback rõ. |
...
| CloudWatch Alarm trực tiếp | Managed alert nhanh, ít code | Khó gắn đủ context recommendation/correlation vào cùng prediction flow | Không chọn làm fail-open chính vì thiếu context audit/annotation của AI flow. |
| Không fallback | Kiến trúc đơn giản nhất | AI down đồng nghĩa không alert; fail hard requirement | Không chọn vì vi phạm yêu cầu fail-open. |
...
| Grafana tự quản trên ECS/EKS | Toàn quyền plugin/database | Vận hành/patching/persistent state tăng scope | Không chọn vì tăng vận hành dashboard vượt mục tiêu W12. |
| CloudWatch Dashboard/Alarm chỉ dùng AWS native | Ít service, tích hợp metric/alarm tốt | Không đáp ứng rõ yêu cầu Grafana annotation overlay | Không chọn làm dashboard chính vì thiếu Grafana annotation overlay. |
...
| Aurora/RDS | SQL/query/reporting linh hoạt, transaction mạnh | Always-on cost, connection/backup/ops cao cho capstone | Không chọn vì audit W12 chỉ cần lookup theo correlation/service. |
| CloudWatch Logs / S3 log files | Ghi log dễ, rẻ/retention linh hoạt | Khó bảo đảm audit schema/query theo correlation và access control nghiệp vụ | Không chọn làm audit system chính vì thiếu access pattern rõ cho audit record. |
...
| SSM Parameter Store SecureString + KMS | Đơn giản/chi phí thấp cho config-secret ít thay đổi | Secret lifecycle/rotation workflow ít chuyên biệt hơn | Không chọn vì cần evidence rõ cho secret access/audit và Grafana token. |
| Environment variable/plain config | Dễ demo nhanh | Rủi ro lộ secret trong repo/log/deploy config | Không chọn vì không đạt security baseline. |
...
| AMP + Grafana chỉ dùng Prometheus alert/query | PromQL và dashboard thống nhất | Không thay thế CloudWatch metrics/logs cho Lambda/SQS/DLQ debugging | Không chọn làm observability độc lập vì Lambda/SQS/DLQ vẫn cần CloudWatch để debug vận hành. |
| OpenTelemetry tracing đầy đủ | Trace quan hệ service rất tốt | Instrumentation/collector/trace backend tăng scope, không phải hard requirement | Không chọn W11 vì correlation ID + CloudWatch logs đã đủ evidence E2E. |
