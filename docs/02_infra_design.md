...
| AI Engine Runtime | ECS Fargate FastAPI service do CDO08 host | Nhận `POST /v1/predict`, detect anomaly và trả recommendation | CDO08 host model serving theo artifact/spec AI bàn giao; dùng internal Application Load Balancer trong private subnet, AI task trong private subnet và IAM SigV4 ở edge layer | CDO08 chịu cost/runtime engine; min/max task, autoscaling và scale-to-zero/cost breaker cần kiểm soát |
...
| Network | ALB là internal ingress trong AI VPC; AI task vẫn nằm private | Serving Adapter Lambda chạy trong AI VPC gọi internal ALB bằng SG-to-SG; SigV4 edge enforcement là hardening riêng trước khi claim full contract auth | Connectivity smoke test |
...
- **AI Engine Runtime:** CDO08 host ECS Fargate theo contract: min 2/max 4, CPU 512, memory 1024 MB, internal Application Load Balancer, AI task trong private subnet, private SG-to-SG path, SigV4 edge hardening và `/health`. CDO08 phải cap concurrency của Prediction Lambda, tôn trọng `429 Retry-After`, giữ request rate thấp hơn 600 requests/min per tenant, và không để retry storm làm quá tải engine.
