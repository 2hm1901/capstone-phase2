...
| AI Engine Runtime | ECS Fargate FastAPI service do CDO08 host | Nhận `POST /v1/predict`, detect anomaly và trả recommendation | CDO08 host model serving theo artifact/spec AI bàn giao; dùng Application Load Balancer trong public subnet, AI task trong private subnet và IAM SigV4 ở service layer | CDO08 chịu cost/runtime engine; min/max task, autoscaling và scale-to-zero/cost breaker cần kiểm soát |
...
|---|---|---|---|
| Ownership | CDO08 host model serving; AI bàn giao artifact/spec | Deploy ECS Fargate service trong private subnet, Application Load Balancer trong public subnet, health check, scaling, rollback bằng CodeDeploy/ECS task definition | Terraform/deploy evidence |
| Network | ALB là public ingress theo diagram mới; AI task vẫn nằm private | Prediction Lambda gọi endpoint ALB bằng HTTPS; ingress ALB phải giới hạn bằng allowlist nếu có thể và AI service enforce IAM SigV4/tenant validation | Connectivity smoke test |
...
- **AI Engine Runtime:** CDO08 host ECS Fargate theo contract: min 2/max 4, CPU 512, memory 1024 MB, Application Load Balancer public ingress, AI task trong private subnet, SigV4 và `/health`. CDO08 phải cap concurrency của Prediction Lambda, tôn trọng `429 Retry-After`, giữ request rate thấp hơn 600 requests/min per tenant, và không để retry storm làm quá tải engine.
